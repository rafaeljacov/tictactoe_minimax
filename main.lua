...
function love.update(dt)
    if cellsPlayed > 4 and not gameOver then
        ...
        elseif board[1][3] ~= '' and board[1][3] == board[2][2] and board[2][2] == board[3][1] then
            winner = board[1][3]
            crossPoints.start.x = boardSize - offset
            crossPoints.start.y = offset
            crossPoints.finish.x = offset
            crossPoints.finish.y = boardSize - offset
        else
            for i = 1, #board do
    ...
    elseif gameOver then
        crossPoints.progress = crossPoints.progress + crossPoints.speed * dt
        if crossPoints.progress > 1 then
            crossPoints.progress = 1
        end
    end
end
...
function love.draw()
    ...
    if gameOver then
        tictactoe.crossWin(crossPoints.start, crossPoints.finish)
    end
end
